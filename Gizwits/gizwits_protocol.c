...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagwork_mod)
    {
        bitFieldBitLen += work_mod_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagwindspeed)
    {
        ...
        info->num++;
        dataPoints->valuewindspeed = gizVarlenDecompressionValue(bitFieldOffset,windspeed_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += windspeed_LEN;
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagwork_mod)
    {
        info->event[info->num] = EVENT_work_mod;
        info->num++;
        dataPoints->valuework_mod = gizVarlenDecompressionValue(bitFieldOffset,work_mod_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += work_mod_LEN;
    }


    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagwemdu_kongzhi)
    {
        info->event[info->num] = EVENT_wemdu_kongzhi;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuewemdu_kongzhi,(uint8_t *)&issuedData[byteFieldOffset],wemdu_kongzhi_LEN);
        byteFieldOffset += wemdu_kongzhi_LEN;
        dataPoints->valuewemdu_kongzhi = gizX2Y(wemdu_kongzhi_RATIO,  wemdu_kongzhi_ADDITION, elongateP0FormTmp.devStatus.valuewemdu_kongzhi); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuewindspeed != cur->valuewindspeed)
    {
        ...
        gizwitsProtocol.waitReportDatapointFlag.flagwindspeed = 1;
        ret = 1;
    }
    if(last->valuework_mod != cur->valuework_mod)
    {
        GIZWITS_LOG("valuework_mod Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagwork_mod = 1;
        ret = 1;
    }
    if(last->valuewemdu_kongzhi != cur->valuewemdu_kongzhi)
    {
        GIZWITS_LOG("valuewemdu_kongzhi Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagwemdu_kongzhi = 1;
        ret = 1;
    }
    ...
    if(last->valueshidu != cur->valueshidu)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            ...
            gizwitsProtocol.waitReportDatapointFlag.flagshidu = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagwindspeed)
    {
        ...
        {
            GIZWITS_LOG("[ERROR] valuewindspeed Error , Illegal Overstep\n");
            return -1;
        }
        else
        {
                gizVarlenCompressValue(bitFieldOffset,windspeed_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuewindspeed);
                bitFieldOffset += windspeed_LEN;
        }
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagwork_mod)
    {
        if(dataPoints->valuework_mod >= work_mod_VALUE_MAX)
        {
            GIZWITS_LOG("[ERROR] valuework_mod Error , Illegal Overstep\n");
            return -1;
        }
        else
        {
                gizVarlenCompressValue(bitFieldOffset,work_mod_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuework_mod);
                bitFieldOffset += work_mod_LEN;
        }
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagwemdu_kongzhi)
    {
        devStatusTmp.valuewemdu_kongzhi = gizY2X(wemdu_kongzhi_RATIO,  wemdu_kongzhi_ADDITION, dataPoints->valuewemdu_kongzhi); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuewemdu_kongzhi,wemdu_kongzhi_LEN);
        byteFieldOffset += wemdu_kongzhi_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagshidu)
    {
        devStatusTmp.valueshidu = gizY2X(shidu_RATIO,  shidu_ADDITION, dataPoints->valueshidu); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueshidu,shidu_LEN);
        byteFieldOffset += shidu_LEN;
    }
